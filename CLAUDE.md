...
- **`next.html`** and **`brief.html`** — **the two stable links.** Both are tiny redirect pages, not copies: `next.html` forwards to the upcoming session's full plan and `brief.html` to its brief, so the URL you hand out never changes and no content is duplicated. Today counts as next all day; if every session is in the past they point at the most recent one.
- **`<plan>.html`** and **`<plan>-brief.html`** — one pair per file in `plans/` with a `PLAN_META` entry. The full plan is the run-sheet in full; the **brief** is built for a phone at the ground: a collapsed **Logistics** accordion, then the run sheet drawn as a **timeline** — time down the page, and a stretch where several things happen at once splits into that many columns. Each block shows only what you need to *run* it (setup and cues, taken from the plan's own Activities entry), with a **Details** button opening a modal of description, coaching objectives and progressions, and a link icon deep-linking into the full plan. Every heading on every page carries an anchor id, which is what those deep links use.

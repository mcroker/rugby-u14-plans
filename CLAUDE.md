...
- **Crediting the source repository.** Setting `source` (`url` and `text`) in `club.json` puts a quiet one-line strip above every page header. It is **off unless set**, and the CSS for it only ships on pages that carry one, so a site not using it is byte-for-byte as if the feature did not exist. Used by the framework repo for its own example site; a club site normally leaves it out.

...
**Setup:** see `activities.md`. Run in parallel with the warm-up: with ~28 players left after the hookers peel off, rotate **three groups of ~9 through the diamond, ~5 min each**, each group coming straight out of the warm-up/handling work and going straight back to it.
...
- Drop to two rotations of ~7 min if the group is slow to settle on the first session back.
...
- **The tackle zone now starts at +0, alongside the warm-up, so the first rotation goes into contact cold.** Have each group do ~60 seconds of their own movement prep as they arrive at the diamond, and keep the first reps of each rotation at walking pace before going full speed. If that feels wrong on the day, run the tackle zone from +5 instead and let the first group warm up with everyone else.

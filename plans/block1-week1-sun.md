# Block 1, Week 1 — Sunday session

## Session details

| | |
|---|---|
| **Date/Time** | Sun 6 Sep 2026 — time **TBC** (check Spond nearer the date; see `calendar.md`) |
| **Location** | Full pitch (home) |
| **Coaches** | *(fill in on the night)* |
| **Attendance** | *(fill in on the night)* |
| **Session objective** | Season opener. Introduce individual tackle skills + communication (defence base); introduce lineout positioning, laws, and the hooker throw-in (the squad's very first lineout exposure); introduce blitz defence line-speed reps for the backs. |
| **Resources required** | Balls, bibs, cones, ruck shields. |

## Plan

| Start + duration | Activity | Focus |
|---|---|---|
| +0, 5 min | Player-led warm-up | General movement, ball in hand |
| +5, 10 min | Zoom-Cruise-Boom | Line-speed movement warm-up (defence) |
| +15, 10 min | Hooker throw-in mini skill zone (hookers) / handling reps (everyone else) | Hooker throw-in technique, run in parallel |
| +25, 5 min | Whole-group intervention: lineout positioning + laws | Where to stand; uncontested, no lift (see `laws.md`) |
| +30, 30 min | Main game: touch/tag, lineout restarts | Line-speed scoreboard; bonus for a clean lineout catch leading to a try |
| +30, 30 min (parallel, backs not in lineout work) | Continuous Defence | Defensive alignment while the forwards are in lineout skill work |
| +60, 15 min | Tackle-technique skill zone (Tackle Diamond, controlled) | Individual tackle technique, small-group, full contact |
| +75, back into main game | Main game continues | Bring the tackle-zone groups straight back in |
| +~85, 5 min | Cool-down / close | Recap line-org cues, set up Thursday |

*Timings are indicative — actual session length depends on the confirmed slot (see Date/Time above); scale each block proportionally if the slot is shorter or longer than ~90 minutes.*

## Activities

### Whole-group intervention — Lineout positioning + laws

**Coaching Points:** where each player stands in the line; lineouts are **uncontested** at U14 — no jumping/lifting, min 2 / max 13 players (see `laws.md`).

**Setup:** whole squad, cones marking the 5m and 15m lines.

**Description:** a short (2–3 min), single-voice coaching point delivered to everyone before the main game starts — not a specialist skill zone, everyone needs to hear this once.

### Hooker throw-in (pull-out skill zone)

**Coaching Points:** accuracy, grip, release.

**Setup:** hookers peel off in a small group with a coach; everyone else continues handling work in the same window.

**Description:** repeated throw-ins at increasing distance/accuracy targets (e.g. cones marking front/middle/back of a 5-man line). Straight back into the main group once the essential-skills window ends.

**Progressions:**
- Increase throw distance/height.
- Add a receiver catching at each target.

### Main game — Touch/tag, lineout restarts

**Coaching Points:** line-speed (defence); handling and support lines (attack); using the lineout restart cleanly.

**Setup:** small-sided, full pitch space (Sunday, so no need to compress into a third of a pitch as on Thursdays — see `claude.md`).

**Description:** small-sided touch/tag game where every restart is a lineout throw-in (uncontested). Defence scores points for line-speed (getting up as a line quickly); attack scores a bonus for a clean lineout catch that leads to a try.

**Progressions:**
- Tighten the line-speed scoring window as the game goes on.
- Layer in the **Bang** blitz-trigger call (see `playbook.md`) once the basic shape is running.

**Adaptations:**
- Reduce grid width if line-speed is too easy to beat; widen it if defence is dominating.

### Continuous Defence (backs, parallel to forwards' lineout work)

**Coaching Points:** quick reorganisation and communication under repeated pressure from alternating directions.

**Setup:** see `activities.md`.

**Description:** run as the backs' parallel focus while the forwards unit (plus hookers) is tied up in lineout work — this is the season's first blitz-defence exposure.

### Tackle Diamond

**Coaching Points:** **low — chop!**, **shoulder contact**, **squeeze** (see `activities.md`). Defence "wins" the rep if the tackle is completed over the halfway point of the diamond.

**Setup:** see `activities.md`.

**Description:** controlled, small-group, full-contact tackle-technique reps — short and sharp, then straight back into the main game.

## Notes

- This is the squad's first session of the season and their first-ever lineout exposure — keep contact introductory even though it's a Sunday with more time/space than usual.
- The **B/W/G** lineout calls aren't introduced until Week 2 — this week is positioning and the throw only.
- Scrum's first exposure is Thursday 10 Sep, not this session.
- See `blocks.md` for the one remaining Block 1 open item — Week 5 backs detail.
